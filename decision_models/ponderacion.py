...
def pesos_uniformes(df, **kw):
    n = df.shape[1]; return pd.Series({c: 1/n for c in df.columns})
...
def run_ponderacion():
    upload3     = widgets.FileUpload(accept=".xlsx,.xls,.csv", multiple=False, description="📂 Subir archivo", button_style="primary")
    upload3_out = widgets.Output()
    col_alt3    = widgets.Dropdown(description="Alternativas:", options=[], style={"description_width":"110px"}, layout=widgets.Layout(width="320px"))
    col_crit3   = widgets.SelectMultiple(description="Criterios:", options=[], layout=widgets.Layout(height="140px", width="380px"), style={"description_width":"80px"})
    norm3_method = widgets.Dropdown(description="Normalización previa:", options=list(NORM_FUNC3.keys()), value="(Sin normalizar)", style={"description_width":"150px"}, layout=widgets.Layout(width="400px"))

    rim3_box = widgets.VBox([])
    rim3_inputs = {}

    pond3_select = widgets.SelectMultiple(options=list(PONDERACIONES3.keys()), value=["Uniforme"], description="Ponderaciones:", layout=widgets.Layout(height="200px", width="340px"), style={"description_width":"120px"})
    ahp3_out = widgets.Output()
    ahp3_panel = widgets.VBox([widgets.HTML("<b>Matriz AHP (comparaciones pareadas)</b><br><small>1 = igual importancia, 9 = extremadamente más importante.</small>"), ahp3_out])
    ahp3_panel.layout.display = "none"
    ord3_out = widgets.Output()
    ord3_panel = widgets.VBox([widgets.HTML("<b>Ordenación Simple – Rangos</b><br><small>1 = menos importante, n = más importante.</small>"), ord3_out])
    ord3_panel.layout.display = "none"
    tas3_out = widgets.Output()
    tas3_panel = widgets.VBox([widgets.HTML("<b>Tasación Simple – Puntajes</b>"), tas3_out])
    tas3_panel.layout.display = "none"
    run3_btn = widgets.Button(description="▶ Calcular pesos", button_style="success", layout=widgets.Layout(width="200px"))
    ...
    def _render3_rim():
        ...
        for crit in crit_cols:
            ...
            w_c = widgets.BoundedFloatText(value=round(sugerido_c, 4), min=-1e6, max=1e6, step=0.01, description=f'{crit[:15]} C:', layout=widgets.Layout(width='280px'))
            w_d = widgets.BoundedFloatText(value=round(sugerido_d, 4), min=-1e6, max=1e6, step=0.01, description='D:', layout=widgets.Layout(width='280px'))
    ...
    def _render3_ahp():
        ...
        for (i, j) in combinations(range(n), 2):
            s = widgets.FloatText(value=1.0, step=0.5, description=f"{cols[i]} vs {cols[j]}:", style={"description_width":"260px"}, layout=widgets.Layout(width="380px"))
    ...
    def _render3_ord():
        ...
        for c in cols:
            w = widgets.IntText(value=1, description=f"{c}:", style={"description_width":"200px"}, layout=widgets.Layout(width="320px"))
    ...
    def _render3_tas():
        ...
        cols = L3["crit_cols"]; wmap = {}; rows = []
        for c in cols:
            w = widgets.FloatText(value=1.0, description=f"{c}:", style={"description_width":"200px"}, layout=widgets.Layout(width="320px"))
            wmap[c] = w; rows.append(w)
        L3["tas_widgets"] = wmap
        with tas3_out:
            clear_output(); display(widgets.VBox(rows))
    ...
    def _run3(b):
        with run3_out:
            ...
            if metodo_norm == "Ideal de referencia":
                if not rim3_inputs:
                    print("❌ No se definieron parámetros C y D para RIM.")
                    return
