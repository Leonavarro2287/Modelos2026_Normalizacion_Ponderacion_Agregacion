...
AHP = {
    'criterios': [],
    'alternativas': [],
    'matriz_criterios': None,
    'pesos_criterios': None,
    'consistencia_criterios': None,
    'matrices_alternativas': {},
    'pesos_alternativas': {},
    'global': None,
    'ia_table': {1:0.00, 2:0.00, 3:0.58, 4:0.90, 5:1.12, 6:1.24, 7:1.32, 8:1.41, 9:1.45, 10:1.49, 11:1.51, 12:1.54, 13:1.56, 14:1.57, 15:1.59},
    'comparaciones_criterios': {},
    'comparaciones_alternativas': {}
}

escala_html = HTML("""
<b>Escala Fundamental de Saaty:</b><br>
1 = Igualmente importante | 3 = Moderadamente importante | 5 = Notablemente importante<br>
7 = Muchísimo más importante | 9 = Absolutamente más importante (2,4,6,8 valores intermedios)
""")
...
def mostrar_matriz_completa(matriz, elementos, titulo):
    ...
    df_mat = pd.DataFrame(matriz, index=elementos, columns=elementos)
    df_norm = pd.DataFrame(norm_mat, index=elementos, columns=elementos)
    df_pesos = pd.DataFrame(pesos, index=elementos, columns=["Peso"])

    display(HTML(f"<b>{titulo}</b>"))
    display(HTML("<b>Matriz de comparación:</b>"))
    display(df_mat.round(4))
    display(HTML("<b>Sumas por columna (Σ a_i):</b>"))
...
def crear_interfaz_dropdown(elementos, titulo, callback_guardar, tipo, crit_nombre=None):
    ...
    rows = []
    header = widgets.HBox([
        widgets.Label(value="Comparación en pares", layout=widgets.Layout(width='350px', font_weight='bold')),
        widgets.Label(value="Criterio más importante", layout=widgets.Layout(width='200px', font_weight='bold')),
        widgets.Label(value="Intensidad (1-9)", layout=widgets.Layout(width='150px', font_weight='bold'))
    ])
    rows.append(header)
    ...
    def on_click(b):
        with output_area:
            ...
            if tipo == 'criterios':
                ...
                if crit_nombre not in AHP['comparaciones_alternativas']:
                    AHP['comparaciones_alternativas'][crit_nombre] = {}
            ...
            if cons['RC'] <= 0.10:
                print("✅ RC ≤ 0.10 → Juicios consistentes.")
            else:
                print("⚠️ RC > 0.10 → Juicios inconsistentes. Revisar las comparaciones.")
...
def run_ahp():
    ...
    proposito_input = widgets.Text(value="Selección de la mejor alternativa", description="Propósito:")
    ...
    def on_upload(change):
        with upload_out:
            ...
            display(df.head())
            upload_file.df = df
            print("✅ Archivo cargado. Presione 'Confirmar y construir árbol'.")
    ...
    def construir_arbol(b):
        with arbol_out:
            clear_output()
            if not hasattr(upload_file, 'df'):
                print("❌ Primero sube un archivo.")
                return
            df = upload_file.df
            alternativas_raw = df.iloc[:, 0].dropna().astype(str).tolist()
            criterios_raw = df.iloc[:, 1].dropna().astype(str).tolist()
            alternativas = list(dict.fromkeys([a.strip() for a in alternativas_raw if a.strip()]))
            criterios = list(dict.fromkeys([c.strip() for c in criterios_raw if c.strip()]))
            if not alternativas or not criterios:
                print("❌ No se encontraron suficientes datos.")
                return
            AHP['alternativas'] = alternativas
            AHP['criterios'] = criterios
            display(HTML(f"<h3>Árbol jerárquico</h3>"))
            display(HTML(f"<b>Propósito:</b> {proposito_input.value}<br>"))
            display(HTML(f"<b>Criterios ({len(criterios)}):</b> {', '.join(criterios)}<br>"))
    ...
    def ingresar_criterios(b):
        with crit_out:
            ...
            if not crits:
                print("❌ Primero construye el árbol (Paso 1).")
                return
    ...
    def ingresar_alternativas(b):
        with alt_out:
            clear_output()
            crits = AHP.get('criterios', [])
            alts = AHP.get('alternativas', [])
            if not crits or not alts:
                print("❌ Primero construye el árbol (Paso 1).")
                return
    ...
    def ejecutar_ahp(b):
        with resultados_out:
            ...
            if AHP['pesos_criterios'] is None:
                print("❌ Primero genera la matriz de criterios (Paso 2).")
                return

            criterios_faltantes = [crit for crit in AHP['criterios'] if crit not in AHP['pesos_alternativas']]
            if criterios_faltantes:
                print(f"❌ Faltan generar las matrices de alternativas para: {', '.join(criterios_faltantes)}")
                return

            crits = AHP['criterios']
            alts = AHP['alternativas']
            ...
            ax.set_xlabel("Prioridad global")
            ax.set_title("Ranking AHP")
            ax.invert_yaxis()
    ...
    display(HTML("<h1>📊 Proceso Analítico Jerárquico (AHP) - Thomas Saaty</h1>"))
    display(HTML("<b>1. Cargar archivo y definir propósito</b>"))
    display(upload_file, upload_out)
    display(proposito_input)
